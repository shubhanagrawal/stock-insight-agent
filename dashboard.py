...
def display_dashboard(insights_container, analytics_col):
    """Renders the main dashboard UI from the database."""
        ...
        if st.session_state.active_chart:
            ...
            for _, data in latest_insights.iterrows():
                with st.container(border=True):
                    ...
                    with col2:
                        # --- FIX APPLIED HERE ---
                        # Ensure a valid string is always passed to the link button.
                        link_url = data.get('link') or '#'
                        st.link_button("🔗 Read Full Article", link_url)
                        # -------------------------
    ...
    # --- Analytics Column (rest of the function is unchanged) ---
