...
def initialize_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS insights (
            ...
            link TEXT,  -- <-- FIX: Added the link column
...
def save_specific_insight(article_title, link, company_name, ticker, sentiment_result, event_type, impact_score, key_figures_json):
    ...
    try:
        ...
        # <-- FIX: Added the link to the INSERT statement
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (timestamp, article_title, link, company_name, ticker, sentiment, confidence, event_type, impact_score, key_figures_json))
        
        conn.commit()
        conn.close()
    except Exception as e:
        logging.error(f"Error saving insight to database: {e}", exc_info=True)
